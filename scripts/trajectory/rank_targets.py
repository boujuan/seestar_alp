...
def _evaluate(path: Path, mount_frame: MountFrame) -> TargetReport:
    ...
    # peak elevation approaches 90°. Now that the site-mismatch bug is
    # fixed, pre_check's peak_v_az_degs is reliable, so use it directly:
    # flag if peak az rate exceeds 80% of the mount cap (5 °/s for a
    # 6 °/s plant). Fall back to a geometric peak_el > 78° trigger as
    # a safety net in case the rate calc breaks again.
    MAIN_RATE_DEGS = 6.0
    near_zenith = (
        pre.peak_v_az_degs > 0.8 * MAIN_RATE_DEGS
        or peak_el_deg > 78.0
    )
